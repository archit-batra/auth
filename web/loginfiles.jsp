...
<link href="css/loginfiles.css" rel="stylesheet" type="text/css">
<title>Uploads</title>
<script language="JavaScript">
	function download (id)
	{
		window.open ("loginDownload?fileId="+id, "hiddenFrame");
                //request.getRequestDispatcher("Download"+"?fileId="+id).forward(request,response);
	}
	function deleteFile (id)
	{
		if (confirm("Delete File ID: "+id))
			window.location.replace ("loginfiles.jsp?delFileId="+id);
	}
...
<label id="label"><a href="Logout">Logout</a></label><style>a{color: #4CAF50;} h5{position: absolute; top: 2%; right: 2%;} h4{position: absolute; top: 5%; right: 2%;} #label{position: absolute; top: 9%; right: 2%;} @media screen and (max-width: 900px) {h5{top:9%;}h4{top:12%;}#label{top:16%;}}@media screen and (max-width: 490px) {h5{top:43%;}h4{top:46%;}#label{top:49%;}}</style>    
  ...
  <li><a class="active" href="loginfiles.jsp">Uploads</a></li>
...
<%      String mail =(String)session.getAttribute("mail");
        ...
        System.out.println("mail in loginfiles is "+mail);
        session.setAttribute("mailmail",mail);
        String url = "jdbc:mysql://localhost:3306/nar";
        String driver="com.mysql.jdbc.Driver";

    	String sql = "SELECT email, file_id, file_name, file_size, received FROM logindocuments WHERE email=?";
        ...
        try {
			...
			{java.sql.Statement stmtt = conn.createStatement();
				stmtt.executeUpdate("DELETE FROM logindocuments WHERE file_id = "+delFileId);
			}
...
if(mail == null && mail.isEmpty()) response.sendRedirect(request.getContextPath() + "/Logout.jsp");
    ...
    <form enctype="multipart/form-data" action="loginupload_file.jsp" method="post">
      <input type="text" style="display: none;" placeholder="Full Name" name="fullName" ></p>
      <input type="email" style="display: none;" placeholder="Email" name="email" ></p>
