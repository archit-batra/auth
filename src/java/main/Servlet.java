...
//        d = request.getParameter("division");
        ...
        if("Login".equals(ac))
        {   
        if(!LoginDao.existingmail(e)){
        request.setAttribute("message", "This email is not registered.");
        request.getRequestDispatcher("Login.jsp").forward(request,response);
        }
        if(LoginDao.validate(e,pr)){
        if(LoginDao.validateStatus(e, activ))
        {
        ...
        String id = LoginDao.getData(n,ln,e,hash,p);
        MailUtil.sendEmailRegistrationLink(id, e, has);
        ...
        if(LoginDao.oldpass(mail,op,hp)){
        ...
        {
        UserPojo up=LoginDao.selectUSERbyEmail(e);
        if(up!=null) {
	String has = Utils.prepareRandomString(30);
        LoginDao.updateEmailVerificationHashForResetPassword(e, BCrypt.hashpw(has,GlobalConstants.SALT));
	MailUtil.sendResetPasswordLink(up.getUSER_ID()+"", e, has);
