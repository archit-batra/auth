...
Class.forName(Setup.DB_DRIVER);  
Connection con=DriverManager.getConnection(Setup.DB_URL,Setup.DB_USERNAME,Setup.DB_PASSWORD);  
PreparedStatement ps=con.prepareStatement("select * from users where email=? and status=?");  
ps.setString(1,email);  
...
Class.forName(Setup.DB_DRIVER);  
Connection con=DriverManager.getConnection(Setup.DB_URL,Setup.DB_USERNAME,Setup.DB_PASSWORD);  
PreparedStatement ps=con.prepareStatement("insert into users(name,last_name,email,email_verification_hash,password) values(?,?,?,?,?)");  
//ps.setString(1,alliance);  
//ps.setString(2,division);  
ps.setString(1,name);  
...
public static boolean verifyEmailHash(String user_id, String hash) throws DBException {
		...
		try {
			...
			ps = conn.prepareStatement("select 1 from users where USER_ID = ? and EMAIL_VERIFICATION_HASH = ?");
...
public static void updateStaus(String user_id, String status) throws DBException{
		...
		try {
			Class.forName(Setup.DB_DRIVER);  
                        conn=DriverManager.getConnection(Setup.DB_URL,Setup.DB_USERNAME,Setup.DB_PASSWORD);
			ps = conn.prepareStatement("update users set STATUS = ? where USER_ID = ?");
	...
	public static void updateEmailVerificationHash(String user_id, String hash) throws DBException{
		...
		try {
			Class.forName(Setup.DB_DRIVER);  
                        conn=DriverManager.getConnection(Setup.DB_URL,Setup.DB_USERNAME,Setup.DB_PASSWORD);
			ps = conn.prepareStatement("update users set EMAIL_VERIFICATION_HASH = ?, EMAIL_VERIFICATION_ATTEMPTS = ? where USER_ID = ?");
			ps.setString(1,hash);
			ps.setInt(2,0);
	...
	public static int incrementVerificationAttempts(String user_id) throws DBException{
		...
		try {
			Class.forName(Setup.DB_DRIVER);  
                        conn=DriverManager.getConnection(Setup.DB_URL,Setup.DB_USERNAME,Setup.DB_PASSWORD);
			ps = conn.prepareStatement("update users set EMAIL_VERIFICATION_ATTEMPTS = EMAIL_VERIFICATION_ATTEMPTS + 1 where USER_ID = ?");
			ps.setString(1,user_id);
			ps.executeUpdate();
		...
		try {
			Class.forName(Setup.DB_DRIVER);  
                        conn=DriverManager.getConnection(Setup.DB_URL,Setup.DB_USERNAME,Setup.DB_PASSWORD);
			ps = conn.prepareStatement("select USER_ID, EMAIL, FIRST_NAME, LAST_NAME, EMAIL_VERIFICATION_HASH, EMAIL_VERIFICATION_ATTEMPTS, STATUS, CREATED_TIME from users where USER_ID = ?");
		...
		try {
			Class.forName(Setup.DB_DRIVER);  
                        conn=DriverManager.getConnection(Setup.DB_URL,Setup.DB_USERNAME,Setup.DB_PASSWORD);
			ps = conn.prepareStatement("select USER_ID, EMAIL, FIRST_NAME, LAST_NAME, STATUS, CREATED_TIME from users where EMAIL = ? and PASSWORD = ?");
		...
		try {
			Class.forName(Setup.DB_DRIVER);  
                        conn=DriverManager.getConnection(Setup.DB_URL,Setup.DB_USERNAME,Setup.DB_PASSWORD);
			ps = conn.prepareStatement("select USER_ID, EMAIL, NAME, LAST_NAME, EMAIL_VERIFICATION_HASH, EMAIL_VERIFICATION_ATTEMPTS, STATUS, CREATED_TIME from users where EMAIL = ?");
			ps.setString(1, inputEmail);
			res = ps.executeQuery();
			if (res != null) {
				while (res.next()) {
